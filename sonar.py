...
def getNewBoard():
    # Create a 60 x 15 board data structure.
    board = []
...
def getRandomChests(numChests):
    # Creates a list of chest data structures (two-item lists of x, y int coordinates).
    chests = []
...
def makeMove(board, chests, x, y):
    # Change the board structure with a sonar device character.
    # Remove treasure chests from the chests list as they are found.
    # Returns False if this is an invalid move.
    # Otherwise returns the string of the result of this move.
    smallestDistance = 100 # Any chest will be closer than 100
    for cx, cy in chests:
        distance = math.sqrt((cx - x) * (cx - x) + (cy - y) * (cy - y))

        if distance < smallestDistance: # We want the closest tresure chest.
            smallestDistance = distance

    smallestDistance = round(smallestDistance)

    if smallestDistance == 0:
        # xy is directly on a trasure chest!
        chests.remove([x, y])
...
how far away the nearest chest is, or an X if it beyond the sonar device's 
...
3 because the closest chest is 3 spaces away.
    ...
    4 ``````~`~~`~`~~`C``~~~``~`~~~```~ 4
      ...
      012345678901234567890123456789012
                1         2         3
(In the real game, the cheasts are not visible in the ocean.)
...
The treasure chest don't move around. Sonar devices can detect treasure chests
    ...
    while sonarDevices > 0:
        ...
        if moveResult == False:
            ...
            if moveResult == 'You have found a sunken treasure chest!':
                # Update all the sonar devices that are currently on the map.
